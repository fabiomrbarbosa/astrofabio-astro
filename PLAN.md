...
- [x] Hero background images replaced with CSS orb blobs in `HeroBackground.astro` — `orb1`/`orb2` props accept any CSS color value; `flip` prop swaps orb positions (so `::after` / orb2 always paints on top); `short` prop sets height to `70vh` instead of `100vh` (used on About page). SVG files and `preloadImage` prop removed. `.hero-background` has `overflow-x: clip` to clip horizontal orb bleed; height is fixed at `100vh` (not `inset: 0`) with a `mask-image` bottom fade (`black 50% → transparent 100%`) so orbs never bleed into sections below the fold. Orb blur uses `clamp(60px, 6vw, 140px)` so softness scales with viewport width; same clamp applied to `BookingForm`'s inline orbs.
